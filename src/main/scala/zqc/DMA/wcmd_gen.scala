...
case class wcmd_gen(dmaInfConfig: dmaInfBundleConfig, bufferConfig : bufferBundleConfig) extends Component{
  val io = new Bundle{
    //--- cfg regs
    val cfg_dar = in(UInt(32 bits))
    val cfg_trans_xsize = in(UInt(16 bits))
    val cfg_trans_ysize = in(UInt(16 bits))
    val cfg_da_ystep = in(UInt(16 bits))

    //--- dma status
    val dma_cmd_sof = in(Bool())
    val dma_cmd_end = out(Bool())
  ...
  val dma_sta = Reg(UInt(2 bits)) init(0)
  val s_idle = 0
  val s_req = 1
  val s_data = 2
  val s_chk = 3
  ...
  val remain_bnum_rd = Reg(SInt(17 bits)) init(0)
  val remain_bnum_send = Reg(SInt(17 bits)) init(0)

  val end_bcnt = dma_addr(0,2 bits) + io.cfg_trans_xsize(0,2 bits) + 1

  val dma_1d_go = io.dma_busy & (io.buffer.buf_buf_word(0,5 bits) =/=0)

  val dma_w_req = (dma_sta === s_req)
  io.dma_inf.dma_w_req := dma_w_req

  val dma_w_addr = dma_addr
  io.dma_inf.dma_w_addr := dma_w_addr

  val dma_w_len = io.cfg_trans_xsize
  io.dma_inf.dma_w_len := dma_w_len

  val send_a_cmd = dma_w_req & io.dma_inf.dma_w_ack
  val send_a_data = io.dma_inf.dma_w_dvld & io.dma_inf.dma_w_dack
  val fir_wdata = send_a_data & (remain_bnum_send(0,16 bits).asUInt === io.cfg_trans_xsize)

  io.dma_cmd_end := False
    ...
    switch(dma_sta){
      is(s_idle){
        ...
        when(dma_1d_go){
          dma_sta := s_req
          dma_addr := io.cfg_dar
          dma_ycnt := io.cfg_trans_ysize
        }
      }
      is(s_req){
        when(io.dma_inf.dma_w_ack){
          dma_sta := s_data
          dma_addr := dma_addr + io.cfg_trans_xsize + 1
          remain_bnum_send := Vec(io.cfg_trans_xsize,U(0,1 bits)).asBits.asSInt
        }
      }
      is(s_data){
      ...
      is(s_chk){
        when(dma_1d_go){
          dma_addr := dma_addr + io.cfg_da_ystep
          dma_sta := s_req
        }
      }
    }
  ...
  io.buffer.buf_err := False
  when(!io.rstn){
    io.buffer.buf_err := False
  }otherwise{
    when((!io.dma_busy) &&(!io.buffer.buf_empty)){
      io.buffer.buf_err := True
    }otherwise{
  ...
  when(!io.rstn){
    remain_bnum_rd := S"17'h1_0000"
  }otherwise{
    when(send_a_cmd){
      remain_bnum_rd := io.cfg_trans_xsize.asSInt.resize(17)
    }otherwise{
  ...
  val nxt_buf_bcnt  = Reg(SInt(4 bits)) init(0)

  val sub_bcnt = Reg(UInt(3 bits)) init(0)
  ...
  io.dma_inf.dma_w_dvld := buf_4b | last_best_vld
  io.dma_inf.dma_wdata := Vec(wd0,wd1,wd2,wd3).asBits

  when(fir_wdata){
    io.dma_inf.dma_wbe := fir_wbe.asUInt
  }otherwise{
    when(dma_wd_last){
      io.dma_inf.dma_wbe := last_wbe.asUInt
    }otherwise{
      io.dma_inf.dma_wbe := U"4'b1111"
    }
  }
